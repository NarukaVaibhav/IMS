...
export class AddFeedbackComponent implements OnInit {
  ...
  // public rows: Array<{question:string,select:string}> = [];
  ...
  ngOnInit() {
    ...
    this.serve.batchdata().subscribe((data) => {
      ...
      for(let key in data["results"]){
       ...
       err=>{this.toastr.error('Check Internet Connection', 'Something Went Wrong', {
        timeOut: 2000
      });
    }
      )
...
submit(){
  ...
  const arr = this.new_results.filter(c=>(c.id==this.batch))
  ...
  this.feed=
   {"questions": this.questions,
    ...
    "answers":{}
        ...
        this.serve.senddata(this.feed).subscribe(
          res=> {
